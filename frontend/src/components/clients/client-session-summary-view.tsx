...
export function ClientSessionSummaryView({ sessionId }: { sessionId: number }) {
  const { data, error, isLoading } = useQuery({
    queryKey: ["trainee-session-summary", sessionId],
    queryFn: () => api.traineeSessionSummary(sessionId),
    retry: false
  });
      ...
      <div className="flex min-h-screen items-center justify-center p-6">
        <Card className="max-w-md">
          <CardBody className="space-y-4">
            ...
            <a className="inline-flex min-h-10 items-center justify-center rounded-md bg-brand px-4 py-2 text-sm font-semibold text-white" href={routes.clientPortal}>
              Back to client hub
            </a>
          </CardBody>
        </Card>
  ...
  return (
    <main className="page-wrap">
      <header className="page-titlebar flex flex-wrap items-end justify-between gap-4">
        ...
        <a className="inline-flex min-h-10 items-center justify-center gap-2 rounded-md border border-line bg-white px-4 py-2 text-sm font-semibold text-ink transition hover:bg-panel" href={routes.clientPortal}>
          <ArrowLeft size={16} />
          Back to hub
        </a>
      </header>
      ...
      <section className="grid gap-4 md:grid-cols-4">
        <SummaryMetric icon={<Dumbbell size={18} />} label="Client" value={String(data.total_clients)} />
      ...
      <section className="space-y-4">
        {data.clients.map((client) => (
          <SessionSummaryCard client={client} key={client.client_id} readOnly onSave={() => undefined} />
        ))}
      </section>
    </main>
  );
